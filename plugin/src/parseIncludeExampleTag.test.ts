...
test("it should parse tag with file path and single line", () => {
	...
	expect(result.parsedSelector).toHaveLength(1);
	expect(result.parsedSelector?.[0]).toEqual({
		type: "single",
		isExclusion: false,
		line: 5,
	});
...
test("it should parse exclusions", () => {
	...
	expect(result.path).toBe("path/to/file");
	expect(result.parsedSelector).toHaveLength(2);
	expect(result.parsedSelector?.[1]).toEqual({
		type: "range",
		isExclusion: true,
		start: 5,
		end: 7,
...
test("it should handle multiple selections with colon syntax", () => {
	...
	expect(result.path).toBe("path/to/file");
	expect(result.parsedSelector).toHaveLength(3);
	expect(result.parsedSelector?.[0]).toEqual({
		type: "range",
		isExclusion: false,
		start: 2,
	...
	expect(result.parsedSelector?.[2]).toEqual({
		type: "range",
		isExclusion: false,
		start: 15,
		end: 20,
...
test("it should handle complex mixed selections", () => {
	...
	expect(result.path).toBe("path/to/file");
	expect(result.parsedSelector).toHaveLength(4);
	expect(result.parsedSelector?.[0]).toEqual({
		type: "range",
		isExclusion: false,
		start: 1,
	...
	expect(result.parsedSelector?.[3]).toEqual({
		type: "single",
		isExclusion: true,
		line: -1,
	});
